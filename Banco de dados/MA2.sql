...
foreign key (cpf) references participante(cpf));
/* TABELAS CRIADAS */
/* CRIANDO A TABELA PARA O TRIGGER */
create table logParticipantes
(id int not null primary key auto_increment,
date date not null,
hora time not null);
/* CRIANDO  O TRIGGER */
DELIMITER //
create trigger quantiParticipantes before insert on participante
for each row
begin
	insert into logParticipantes values(null, current_date, current_time);
end;
//
select * from logParticipantes
/* TRIGGER CRIADO */
/* CRIANDO STORED PROCEDURE INSERÇÃO PARTICIPANTE */
DELIMITER //
create procedure atv_participante_ins(p_cpf varchar(15), p_nome varchar (50))
begin
	if((p_cpf != '') and (p_nome != '')) then
		insert into participante values(p_cpf, p_nome);
	else
		select 'Cpf e Nome devem ser informados!' as mensagem;
	end if;
end;
//
call atv_participante_ins('666.666.666-66', 'Marcio Reis');
/* FIM STORED PROCEDURE INSERÇÃO */
/* CRIANDO STORED PROCEDURE QUANTIDADE PARTICIPANTES */
DELIMITER //
create procedure atv_participante_quant()
begin
	select count(id) as QuantidadeParticipantes from logParticipantes;
end;
//
call atv_participante_quant;
/* FIM STORED PROCEDURE QUANTIDADE PARTICIPANTES */
/* CRIANDO STORED PROCEDURE CHECANDO CPF */
DELIMITER //
create procedure atv_participante_cpf(p_cpf varchar(15))
begin
	if exists (select * from participante where cpf = p_cpf) then
		select 'Cpf já cadastrato' as mensagem;
	else 
		select 'Cpf não cadastrado' as mensagem;
	end if;
end;
//
call atv_participante_cpf('666.666.666-66');
/* FIM STORED PROCEDURE CHECANDO CPF */
/* CRIANDO VIEWS PARA O CÓDIGO */
create view vw_questaoAlter as
select alternativa.questao as num_questao, 
       questao.descricao as pergunta, 
       alternativa.descricao as alternativa
from questao
join alternativa
on alternativa.questao = questao.codigoQ;
select * from vw_questaoAlter;
/* PRIMEIRA VIEW CRIADA */
/* CRIANDO A SEGUNDA VIEW */      	
create view vw_participantePontos as
select participante.cpf as cpf,
       participante.nome as nome,
       participantequestao.pontuacao as pontuacao
from participante
join participantequestao
on participantequestao.cpf = participante.cpf;
select * from vw_participantePontos;
/* SEGUNDA VIEW CRIADA */
/* CRIANDO A TERCEIRA VIEW */       
create view vw_pontuacaofinal as
select participantequestao.cpf as cpf, 
	   participante.nome as nome,
       sum(pontuacao) as total_pontos
from participantequestao
join participante
on participante.cpf = participantequestao.cpf
group by cpf;
select * from vw_pontuacaofinal;
/* VIEWS CRIADAS */
